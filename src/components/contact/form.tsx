...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (response.success) {
      ...
      // Auto-reset the visual success window state after 4 seconds
      setTimeout(() => {
        setSubmitted(false);
      }, 4000);
    } else {
  ...
  return (
    <div className="bg-white rounded-3xl p-8 sm:p-10 border border-gray-200/60 shadow-md relative">
      ...
      {submitted ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-1.5">
              ...
              <input 
                ...
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-[#0D7C66] focus:bg-white transition-all disabled:opacity-60"
          ...
          <input 
            ...
            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-950 placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-[#0D7C66] focus:bg-white transition-all disabled:opacity-60"
            placeholder=""
          />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <textarea 
            ...
            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-950 placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-[#0D7C66] focus:bg-white transition-all resize-none disabled:opacity-60"
          ...
          <button
            type="submit"
            disabled={isSubmitting}
            className="w-full bg-[#0D7C66] hover:bg-[#41B3A2] disabled:bg-gray-300 text-white font-bold py-4 rounded-xl shadow-sm transition-colors duration-150 text-sm tracking-wide uppercase flex items-center justify-center gap-2 cursor-pointer disabled:cursor-not-allowed"
          >
            {isSubmitting ? (
              <>
